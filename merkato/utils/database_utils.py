...
def create_merkatos_table():
    try:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS merkatos
                    (exchange text, exchange_pair text, pair text, spread text, profit_limit integer, last_order text, ask_reserved_balance float, bid_reserved_balance float)''')
...
def insert_merkato(exchange, exchange_pair='tuxBTC_ETH', pair='BTC_ETH', spread='.1', profit_limit=10, last_order='', bid_reserved_balance=0, ask_reserved_balance=0):
    try:
        conn = sqlite3.connect('merkato.db')
    except Exception as e:
        print(str(e))
        ...
        c.execute("""INSERT INTO merkatos 
                    (exchange, exchange_pair, pair, spread, profit_limit, last_order, ask_reserved_balance, bid_reserved_balance) VALUES (?,?,?,?,?,?,?,?,?,?)""", 
                    (exchange, exchange_pair, pair, spread, profit_limit, last_order, ask_reserved_balance, bid_reserved_balance))
        conn.commit()
        conn.close()
