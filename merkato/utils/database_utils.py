...
def create_exchanges_table():
		try:
				...
				c.execute('''CREATE TABLE IF NOT EXISTS exchanges
										(exchange text, public_api_key text, private_api_key text	)''')
				c.execute('''CREATE UNIQUE INDEX id_exchange ON exchanges (exchange)''')
				conn.commit()
				conn.close()

def insert_exchange(exchange, public_api_key='', private_api_key=''):
		try:
				conn = sqlite3.connect('merkato.db')
		except Exception as e:
				print(str(e))
				...
				c.execute("""INSERT INTO exchanges 
										(exchange, public_api_key, private_api_key) VALUES (?,?,?)""", 
										(exchange, public_api_key, private_api_key))
				conn.commit()
				conn.close()
